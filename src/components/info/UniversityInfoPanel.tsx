  ...
  return (
    <div className={`rounded-xl border border-slate-200 border-t-4 bg-white p-4 shadow-soft sm:p-6 ${theme.topBorder}`}>
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          {category && (
            <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-medium ${theme.badge}`}>
              {category.shortNameBn}
            </span>
          )}
          <h2 className="mt-2 text-lg font-bold text-navy-900 sm:text-xl">{university.nameBn}</h2>
          <p className="text-xs text-slate-400">{university.nameEn}</p>
        </div>
      ...
      {university.introBn && <p className="mt-3 text-sm text-slate-600">{university.introBn}</p>}
      ...
      <div className="mt-5 divide-y divide-slate-200">
        <InfoSection title="গুরুত্বপূর্ণ তারিখ" isEmpty={false}>
        ...
        <InfoSection title="আসন সংখ্যা" isEmpty={!activeUnit.seats}>
          {activeUnit.seats && (
            <div>
              {activeUnit.seats.total !== null && (
                <p className="text-sm text-slate-700">
                  মোট আসন: <span className="font-semibold">{toBanglaNumber(activeUnit.seats.total)}</span>
                </p>
              )}
              {activeUnit.seats.breakdown && activeUnit.seats.breakdown.length > 0 && (
                <ul className="mt-2 divide-y divide-slate-200 text-sm text-slate-600">
                  {activeUnit.seats.breakdown.map((b) => (
                    <li key={b.nameBn} className="flex justify-between py-1.5">
                      <span>{b.nameBn}</span>
                      <span className="font-medium">{toBanglaNumber(b.count)}</span>
                    </li>
                  ))}
                </ul>
              )}
        ...
        <InfoSection title="আবেদন যোগ্যতা" isEmpty={!activeUnit.eligibility?.descriptionBn && !activeUnit.eligibility?.points?.length}>
          {activeUnit.eligibility?.descriptionBn && (
            <p className="text-sm text-slate-700">{activeUnit.eligibility.descriptionBn}</p>
          )}
        ...
        <InfoSection title="পরীক্ষার ধরন" isEmpty={!activeUnit.examPattern}>
          <p className="text-sm text-slate-700">{activeUnit.examPattern}</p>
        </InfoSection>

        <InfoSection title="বিষয় ও প্রশ্নসংখ্যা" isEmpty={activeUnit.subjects.length === 0}>
          <ul className="divide-y divide-slate-200 text-sm">
            {activeUnit.subjects.map((s) => (
              <li key={s.nameBn} className="flex items-center justify-between py-1.5">
                <span className="text-slate-600">{s.nameBn}</span>
                <span className="font-semibold text-navy-800">{toBanglaNumber(s.marks)}</span>
              </li>
            ))}
          </ul>
        </InfoSection>

        <InfoSection title="ফলাফল নির্ণয় পদ্ধতি" isEmpty={!activeUnit.resultMethod}>
          <p className="text-sm text-slate-700">{activeUnit.resultMethod}</p>
        </InfoSection>

        <div className="pt-4">
          <h4 className="mb-2 text-sm font-bold text-navy-800">অফিসিয়াল সার্কুলার</h4>
...
function DateBox({ label, value }: { label: string; value: string | null }) {
  return (
    <div className="rounded-lg bg-slate-50 px-3 py-2.5">
      ...
      <p className="mt-0.5 text-sm font-semibold text-navy-800">{value ?? "তথ্য প্রকাশিত হয়নি"}</p>
    </div>
  );
}
